...
class DiffusionTransformer(nn.Module):
    ...
    def update_n_sample(self):
        if self.num_timesteps == 100:
            ...
            self.n_sample = [1, 3] + [6, 6, 4, 4] * 49 + [6, 9]
    ...
    def cf_predict_start(self, log_x_t, cond_emb, cf_cond_emb, t):
        ...
        cf_log_x_recon = self.predict_start(log_x_t, cf_cond_emb.type_as(cond_emb), t)[:, :-1]
    ...
    def p_pred(self, log_x, cond_emb, cf_cond_emb, t):             # if x0, first p(x0|xt), than sum(q(xt-1|xt,x0)*p(x0|xt))
        if self.parametrization == 'x0':
            # log_x_recon = self.predict_start(log_x, cond_emb, t)
            log_x_recon = self.cf_predict_start(log_x, cond_emb, cf_cond_emb, t)
    ...
    def p_sample(self, log_x, cond_emb, cf_cond_emb, t, sampled,
                 to_sample):  # sample q(xt-1) for next step from  xt, actually is p(xt-1|xt)
        model_log_prob, log_x_recon = self.p_pred(log_x, cond_emb, cf_cond_emb, t)

        max_sample_per_step = self.prior_ps  # max number to sample per step
        if t[
            0] > 0 and self.prior_rule > 0:  # prior_rule: 0 for VQ-Diffusion v1, 1 for only high-quality inference, 2 for purity prior
    ...
    def _train_loss(self, x, cond_emb, is_train=True):                       # get the KL loss
        ...
        return log_model_prob, vb_loss, x0_recon
    ...
    def forward(
        ...
        if is_train == True:
            log_model_prob, loss, sample_image_recon = self._train_loss(sample_image, cond_emb)
        ...
        if return_loss:
            out['loss'] = loss 
        self.amp = False
        out['pred_data'] = sample_image_recon
        return out
    ...
    def sample(
        ...
        input = {'condition_token': condition_token,
                ...
                'cf_condition_embed_token': cf_condition_embed,
                'content_logits': content_logits,
                }

        if input['condition_token'] != None:
            batch_size = len(input['condition_token'])                                      # Check if correct batch size
        else:
        ...
        if self.condition_emb is not None:  # do this
            ...
            if input.get('condition_embed_token', None) != None:
                cond_emb = input['condition_embed_token'].float()
                cf_cond_emb = input['cf_condition_embed_token'].float()
            else:
                cond_emb = None
        ...
        if start_step == 0:
            ...
            with torch.no_grad():
                for diffusion_index in tqdm(range(start_step-1, -1, -1)):
                    ...
                    while min(sampled) < self.n_sample[diffusion_index]:
                        log_z, sampled = self.p_sample(log_z, cond_emb, cf_cond_emb, t, sampled, self.n_sample[diffusion_index])
            ...
            with torch.no_grad():
                for diffusion_index in range(start_step-1, -1, -1):
                    t = torch.full((batch_size,), diffusion_index, device=device, dtype=torch.long)
                    log_z = self.p_sample(log_z, cond_emb, cf_cond_emb, t)     # log_z is log_onehot
