...
from src.models.utils.model_utils import MultiHeadAttention
...
class VQVAE(pl.LightningModule):
    def __init__(self, embedding_dim, n_codes, n_hiddens, n_res_layers, downsample, sequence_length, resolution):
        ...
        self.encoder = Encoder(self.n_hiddens, self.n_res_layers, self.downsample)
        self.decoder = Decoder(self.n_hiddens, self.n_res_layers, self.downsample)

        self.pre_vq_conv = SamePadConv3d(self.n_hiddens, self.embedding_dim, 1)
        self.post_vq_conv = SamePadConv3d(self.embedding_dim, self.n_hiddens, 1)
    ...
    def forward(self, x, do_inference=False):
        ...
        model_out = {
            ...
            'gt_data': x,
            'losses': {'recon_loss': recon_loss, 'commitment_loss': vq_output['commitment_loss']},
        }
        return model_out
